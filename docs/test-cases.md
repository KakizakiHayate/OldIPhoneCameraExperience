...
#### 画角クロップ（F2.2）— Phase 2に移動、テスト内容を変更

| ID | テストケース | 期待結果 |
|----|-------------|---------|
| S-F4 | applyCropにCIImageを渡すとクロップされた画像が返ること | 出力の`extent`が入力より小さい |
| S-F5 | クロップ後のアスペクト比が入力と同じであること | 入力と出力のアスペクト比が一致 |
| S-F6 | クロップ後のサイズがcropRatio分だけ縮小されていること | `extent.width == 入力幅 × cropRatio` |

#### 解像度スケーリング（5MP）

| ID | テストケース | 期待結果 |
|----|-------------|---------|
| S-F-DS1 | applyDownscaleで横長画像が5MP相当にスケーリングされること | アスペクト比を維持しつつ2592×1936以内に収まる |
| S-F-DS2 | applyDownscaleで縦長画像のwidth/heightが正しく入れ替わること | ポートレート時はターゲットが1936×2592に入れ替わる |
| S-F-DS3 | 既にターゲットサイズ以下の画像はスケーリングしないこと | 入力と出力のサイズが同一 |
